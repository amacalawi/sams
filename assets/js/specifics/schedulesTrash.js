jQuery(document).ready(function ($) {
    var schedulesTrashTable = $('#trashed-schedule-table-command').bootgrid({
        labels: {
        ...
        schedulesTrashTable.find('.command-restore').on('click', function (e) {
            var id   = $(this).parents('tr').data('row-id'),
                name = $(this).parents('tr').find('td.fullname').text(),
                url  = base_url('schedules/restore/' + id);
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: url,
                success: function (data) {
                    ...
                    // swal("Restored", data.schedule.message, data.schedule.type);
                }
            });
        })
    });
...
function reload_trash_table() {
    $('#trashed-schedule-table-command').bootgrid('reload');
}