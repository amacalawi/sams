jQuery(document).ready(function ($) {
    var privilegesTrashTable = $('#trashed-privilege-table-command').bootgrid({
        labels: {
        ...
        privilegesTrashTable.find('.command-restore').on('click', function (e) {
            var id   = $(this).parents('tr').data('row-id'),
                name = $(this).parents('tr').find('td.fullname').text(),
                url  = base_url('privileges/restore/' + id);
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: url,
                success: function (data) {
                    ...
                    // swal("Restored", data.privilege.message, data.privilege.type);
                }
            });
        })
    });
...
function reload_trash_table() {
    $('#trashed-privilege-table-command').bootgrid('reload');
}