jQuery(document).ready(function ($) {
    var membersTrashTable = $('#trashed-message-templates-table-command').bootgrid({
        labels: {
        ...
        membersTrashTable.find('.command-restore').on('click', function (e) {
            var id   = $(this).parents('tr').data('row-id'),
                ...
                url  = base_url('messaging/templates/restore/' + id);
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: url,
...
function reload_trash_table() {
    $('#trashed-message-templates-table-command').bootgrid('reload');
}