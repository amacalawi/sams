jQuery(document).ready(function ($) {
    var modulesTrashTable = $('#trashed-module-table-command').bootgrid({
        labels: {
        ...
        modulesTrashTable.find('.command-restore').on('click', function (e) {
            var id   = $(this).parents('tr').data('row-id'),
                name = $(this).parents('tr').find('td.name').text(),
                url  = base_url('modules/restore/' + id);
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: url,
                success: function (data) {
                    ...
                    // swal("Restored", data.module.message, data.module.type);
                }
            });
        })
    });
...
function reload_trash_table() {
    $('#trashed-module-table-command').bootgrid('reload');
}