jQuery(document).ready(function ($) {
    var privilegesLevelsTrashTable = $('#trashed-privileges-level-table-command').bootgrid({
        labels: {
        ...
        privilegesLevelsTrashTable.find('.command-restore').on('click', function (e) {
            var id   = $(this).parents('tr').data('row-id'),
                name = $(this).parents('tr').find('td.name').text(),
                url  = base_url('privileges-levels/restore/' + id);
            e.preventDefault();
            $.ajax({
                type: 'POST',
                url: url,
                success: function (data) {
                    ...
                    // swal("Restored", data.privileges-level.message, data.privileges-level.type);
                }
            });
        })
    });
...
function reload_trash_table() {
    $('#trashed-privileges-level-table-command').bootgrid('reload');
}