...
class Monitor extends CI_Model {
    ...
    public function get_dtrlog_am_out($stud_no,$timelog,$timefrom,$timeto){
        $query2 = $this->db->query("SELECT * FROM dtr_log as dtr INNER JOIN members as mem ON dtr.member_id = mem.id WHERE dtr.timelog like '%$timelog%' AND dtr.timelog > '".$timelog." ".$timefrom."' and mem.stud_no = '".$stud_no."' ORDER BY dtr.id DESC LIMIT 0,1"); // dtr.log_type = 'TIME_OUT' AND
    ...
    public function get_dtrlog_pm_in($stud_no,$timelog,$timefrom,$timeto){
        $query2 = $this->db->query("SELECT * FROM dtr_log as dtr INNER JOIN members as mem ON dtr.member_id = mem.id where dtr.timelog like '%".$timelog."%' and dtr.timelog >= '".$timelog." ".$timefrom."' and mem.stud_no = '".$stud_no."' and dtr.mode = 3 ORDER BY dtr.id ASC LIMIT 0,1");
        foreach($query2->result() as $row2 ){
            $timelog = date("H:i:s", strtotime($row2->timelog));
        }
        return $query2->num_rows()==1 ? $timelog : '';
    ...
    public function get_dtrlog_pm_out($stud_no,$timelog,$timefrom,$timeto){
        $query2 = $this->db->query("SELECT * FROM dtr_log as dtr INNER JOIN members as mem ON dtr.member_id = mem.id where dtr.timelog like '%".$timelog."%' and dtr.timelog >= '".$timelog." ".$timeto."' and mem.stud_no = '".$stud_no."' and dtr.mode = 4 ORDER BY dtr.id DESC LIMIT 0,1");
        foreach($query2->result() as $row2 ){
            $timelog = date("H:i:s", strtotime($row2->timelog));
        }
        return $query2->num_rows()==1 ? $timelog : '';
    ...
    public function get_late_in_by_member_id($id)
    {
        ...
        $this->db->from('members as mem');
       // $this->db->join('dtr_time_settings as dtr','mem.schedule_id = dtr.schedule_id');
    ...
    public function select_members_from_level_logs(&$a,&$b,&$c,&$d,&$e,&$f,&$g,&$h,$id)
    {
        $date_from = date("Y-m-d",strtotime(str_replace('/', '-', $a)));
        $date_end = date("Y-m-d",strtotime(str_replace('/', '-', $b)));

        $time_from = date("Y-m-d",strtotime(str_replace('/', '-', $a)));
        $time_to = date("Y-m-d",strtotime(str_replace('/', '-', $b)));
    ...
    $date_end = date("Y-m-d",strtotime(str_replace('/', '-', $b)));
        ...
        elseif ($e=="Late_Only"):
            //
            $this->db->select('mem.id, lvl.levels_id, mem.schedule_id');
            $this->db->from('levels as lvl');
            $this->db->join('members as mem','lvl.levels_id = mem.level');
            $this->db->order_by('mem.level', $f);
            $this->db->group_by('mem.level');
